...
>
> **Next up, in priority order:** Remaining Phase-3 open items, now just two: **3.10** (per-iteration timeout — the kill mechanism already exists in `src/lib/supervise` + `loop.Result.TimedOut` surfaces it; only the orchestrator's recording/halt remains, so this is mostly a Phase-5 concern); **3.12** (usage-limit wait/auto-resume — sleep-to-`reset_at`/`backoff`, `WAITING` state persistence, `max_cycles`/`cycles_used` cap, auto-resume; spec 01 + 09). With 3.11 landed the context-pressure path is complete end-to-end. Recommended next: **3.12** (highest leverage — it is what makes unattended multi-day runs survive a subscription limit; all deps ready: `Classify`/`ResetAt` from 2.3, `state.Usage`/`reset_at` from 1.5, `[usage]` config). 3.10 is small but naturally lands with the Phase-5 orchestrator that owns the run-state machine.
>
...
- External dependencies: `github.com/BurntSushi/toml v1.4.0` (config parsing, TOML);
  ...
  and `src/lib/reconcile` (pure, imports task) add no external dependency. `src/lib/guardrail` (pure, imports nothing) adds no external dependency.
