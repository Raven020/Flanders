...
>
> **Next up, in priority order:** With 4.2 done, the open Phase-4 items are **4.3 plan-completeness check** (the plan-loop EXIT condition — the orchestrator calls `loop.Driver.PlanIterate` repeatedly until this passes; audit recommends a mechanical coverage scan mapping each spec requirement/heading to ≥1 task ref — cheapest, harness-owned ground truth, parallels the test-gate philosophy), then the build-phase TDD flow **4.4** (test agent: ensure a red acceptance test, or mark done+skip-build when one already passes) and **4.5** (per-task test→build→verify), and **4.6** (split pass: a fresh agent turns a `blocked: context-overreach` task + handoff into 2–4 smaller tasks — reuses plan settings, and `PlanIterate`/`composePlanPrompt` are a close template). The remaining Phase-3 item **3.10** (per-iteration-timeout recording) stays orchestrator-coupled. **Phase 5** (orchestration) is the consumer that wires plan↔build, the run-state machine, and 3.8–3.12/3.10 together — the natural home once 4.3–4.6 give it phases to drive. The 4.2 architectural question (branch-in-Iterate vs. separate driver) is RESOLVED: a separate `PlanIterate` with shared helpers (`commit`/`baseSummary`/`newContextGuard`).
>
