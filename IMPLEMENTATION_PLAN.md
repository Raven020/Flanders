...
>
> **Next up, in priority order:** (1) **Phase 3.5 status reconciliation** — agent-status-first, then git-diff + test-gate inference; also writes `status`/`reason` directly when the harness ends a loop, and is the home of the deferred RUNNING-crash git-reconcile (spec 09). The verify gate (3.4) now feeds it the ground-truth verdict. (2) **Phase 3.7 done-detection** — now unblocked: done iff `Verify.Passed()` AND every task `done` AND no stall; the per-loop pieces exist, this composes them. (3) **Phase 3.2 prompt composition** — enrich the minimal composer with dependency outcomes + named spec excerpts. (4) **Phase 3.6 git checkpointing** — commit on progress with the templated message.
>
