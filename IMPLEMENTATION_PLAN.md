...
>
> **Next up, in priority order:** The only remaining Phase-3 open item is **3.10** (per-iteration timeout), and it is mostly a Phase-5 concern — the kill mechanism already exists in `src/lib/supervise` and `loop.Result.TimedOut` surfaces it; only the orchestrator's recording/halt remains. With 3.12 landed the entire usage-limit survival path is complete end-to-end (detect → wait/persist `WAITING` → auto-resume → `max_cycles` cap), so Phase 3 is now effectively complete except for the orchestrator-coupled 3.10. The natural next major work is therefore **Phase 4** (phases & agent classes: **4.1 subagent-class resolution is now DONE**; remaining are 4.2 plan loop, 4.3 plan-completeness check, 4.4/4.5 build flow, 4.6 split) and/or **Phase 5** (orchestration) — the consumer that wires 3.1–3.12 together and is also the natural home for 3.10's recording/halt. ARCHITECTURAL NOTE for 4.2 (discovered this loop): the plan loop is NOT task-selected — unlike `loop.Driver.Iterate`'s existing spine (which always `store.Next()`-selects a task and `composePrompt`s around it), a plan loop operates on the whole spec set to PRODUCE/UPDATE task files, and `composePrompt` currently REQUIRES a non-nil `*task.Task` (the `t==nil` path returns `NoWork` before compose). So 4.2 needs a distinct plan-phase composition path (inject `specs/*.md` non-task specs + existing `specs/tasks/*.md` + a decompose-to-smallest-checkable instruction) and a plan-phase select/exit that is driven by the plan-completeness check (4.3, the plan-loop exit condition) rather than by `store.Next()`. Decide: branch inside `Iterate` on phase, or a separate plan driver.
>
