...
>
> **PROGRESS (2026-05-26):** **4.5 per-task build flow DONE** — NEW `src/lib/loop/taskflow.go`: `Driver.RunTask(ctx, iter, stallCount int) (*TaskFlowResult, error)`, the smallest orchestration unit above the single-loop `Iterate` (spec 06 §Phases, spec 07 §Per-task flow). It composes `Iterate` calls into the TDD test→build→verify sequence for ONE task and returns a terminal `Disposition` the Phase-5 orchestrator routes on without re-deriving. FLOW: a bounded TEST phase (`Iterate(ctx,"test",…)`) reads `Result.TestVerdict` — Satisfied → `DispDone` (skip build, spec 07 branch 2); Blocked → `DispBlocked` (carry `reason`, drain); RedReady → proceed; NoRedTest → re-run the test loop up to `defaultMaxTestAttempts`=2 then `DispNoRedTest` (refuse to build against no red test — the TDD invariant) — then a BUILD phase (`Iterate(ctx,"build",…)`) loops until the task reaches a terminal status: `Reconcile.To==done` → `DispDone` (with `Verified` = the final gate passed), `==blocked` → `DispBlocked`. ARCHITECTURE: `RunTask` runs MULTIPLE loops itself (the per-task inner loop is local to one task) but the run-state machine stays Phase-5's — the two coupling points are passed through explicitly, not duplicated: `iter` (so the per-phase `[guardrails].max_iterations` cap is honored across the inner loops via `guardrail.MaxIterationsReached`, → `DispMaxIterations`; also feeds the checkpoint `{iter}`) and `stallCount` (threaded through EVERY inner loop via `guardrail.StallStep`/`StallTripped` → `DispStalled`, then returned on `TaskFlowResult.StallCount` for the orchestrator to persist). `RunTask` never touches state.json. Test loops count toward the same budget/stall as build loops (spec 06 frames test→build→verify all within the build phase; the finer plan-vs-build apportionment stays spec-06 OPEN). GROUND TRUTH not self-report: the disposition derives from `TestVerdict`/`Reconcile`/the git work-signal/the loop outcome; a build-phase agent `done` without a green gate is respected on disk (agent-status-first, spec 02) but surfaced via `Verified=false` so the orchestrator's run-level done-gate (spec 01) catches a false done. Non-success outcomes end the flow with the matching disposition (`dispositionForOutcome`: TimedOut → `DispTimeout`, OutcomeUsageLimit → `DispUsageLimit`, else `DispError`) so the orchestrator waits/resumes or halts. `TaskFlowResult` carries Disposition/Task/AllDone/Reason/TestVerdict/Verified/Iterations/StallCount/Results/Last. 9 new tests in `taskflow_test.go` (headline red→green→verified happy path via a `test -f built` sentinel gate + a phase-aware stub keyed off the compose role header; satisfied-skips-build; blocked-drains; no-work; no-red-test-retries-then-gives-up; stalls; max-iterations; usage-limit-returns; `dispositionForOutcome` unit table). NOT wired into a run loop — the Phase-5 orchestrator is the consumer that calls `RunTask` across the drain. `go build/vet/test -race ./...` green across 17 packages. `Version` 0.0.27, tag 0.0.27.
>
> **PROGRESS (2026-05-26):** **5.1 phase machine + 5.2 full autonomy + 5.3 termination/summary DONE** — NEW package `src/lib/orchestrate` (`orchestrate.go` + `orchestrate_test.go`), the bare-`flanders` run loop, plus wired into `src/cmd/flanders/main.go runOrchestrate`. `orchestrate.Orchestrator.Run(ctx) (*Summary, error)` is the outermost coordinator that composes the Phase-3/4 primitives into a whole run: **plan phase** (`PlanIterate` until `PlanComplete().Complete()`), **build drain** (`RunTask` across actionable tasks until DispNoWork), and **drain-then-batch-replan** (spec 06, locked): a build snag is marked `blocked` by the agent and the drain moves on; only when the drain is exhausted (every task done|blocked) does the orchestrator run **exactly ONE focused plan loop** per drain boundary to resolve the blocks, then resume build — never one phase switch per snag (test `TestDrainThenSingleBatchReplan` asserts two blocked tasks trigger exactly one re-plan, planN==2). **Run-state machine**: owns state.json, persisting at every transition (phase change, loop start/end via iter counts, guardrail trip, usage wait/resume, done). **Done-detection** (spec 01/06, locked): DONE iff the harness's own run-level test gate is green AND all tasks done AND no stall; the agents' reports are advisory — the gate (via `src/lib/verify`, a stubbable `verify` seam) is ground truth, and an all-tasks-done-but-red-suite case halts as a FALSE-DONE backstop. **Guardrail halts**: stall, max-iterations, plan-stall, no-progress-replan, degraded no-red-test, and usage `on_limit=halt` — each leaves HALTED + a reason on the `Summary`. **Usage wait** (spec 09): a `DispUsageLimit`/`OutcomeUsageLimit` loop calls `usage.Waiter.HandleLimit` (persist WAITING, sleep to reset, resume); a restored WAITING resumes via `Waiter.Resume`. **Resume** (spec 09 §recovery): restored DONE/HALTED report-and-stop (a guardrail halt is the operator's to clear — no silent auto-retry), restored RUNNING crash-reconciles (`reconcileResume` normalizes any `active` task back to `pending` via `src/lib/reconcile` so the half-finished loop is re-derived, not trusted), restored PAUSED resumes. **DECISION — iteration-budget apportionment (spec 06 OPEN, now resolved):** `[guardrails].max_iterations` is a PER-PHASE cap — plan loops bounded by `state.iter.plan`, build loops by `state.iter.build`, each vs. max_iterations (so a runaway plan phase can't exhaust the build budget); documented in the package doc, swappable for a global/split cap without touching the loop primitives. **Autonomy (5.2)**: the pipeline runs unattended with no per-cycle approval; it pauses only on a guardrail HALT or usage WAIT. **Termination (5.3)**: `Summary{RunState,HaltReason,HaltTask,TasksDone,TasksBlocked,PlanIters,BuildIters,TotalIters,Duration,CostUSD}` printed by `main.printSummary`. **TESTING**: a scriptable `fakeDriver` (the `driver` interface = `PlanIterate`/`PlanComplete`/`RunTask`, satisfied by `*loop.Driver`) performs the SAME on-disk task-file mutations the real agent+reconcile would, so the orchestrator reads genuine ground truth from disk while the test controls loop outcomes without a live `claude` — 10 tests: happy-path, drain→single-batch-replan, no-progress-replan-halts, false-done-halts, stall-halts, max-iter-halts, usage-wait-then-resume (cycles_used==1), usage-halt-mode, resume-DONE/HALTED-no-op, resume-RUNNING-reconciles-active, plan-phase-stall. `main.go` wires a signal-aware ctx (Ctrl-C → graceful stop, state on disk resumes). **DEFERRED (documented, not stubbed):** (a) crash-resume git-diff ENRICHMENT (crediting work a crashed loop landed but never flipped status for) beyond the safe active→pending normalize — spec 09 future refinement; (b) feeding blocked-task REASONS into the focused re-plan prompt + using the 4.6 split pass for `context-overreach` blocks (re-plan currently sees blocked tasks via the existing-tasks summary's status); (c) `DispNoRedTest` currently HALTS — deferring it to a re-plan is a possible future refinement. `go build/vet/test -race ./...` green across 18 packages. `Version` 0.0.28, tag 0.0.28.
>
> **Next up, in priority order:** Phase 5 (orchestration) is DONE — the bare `flanders` command now drives the full plan→build pipeline. The remaining gaps before a complete v1: **4.6** (split pass: a fresh agent turns a `blocked: context-overreach` task + handoff into 2–4 smaller tasks; reuses `plan` settings; `PlanIterate`/`composePlanPrompt` are a close template) — and wiring the split pass into the orchestrator's re-plan path for `context-overreach` blocks; **3.10** (per-iteration-timeout recording — `supervise` already kills on timeout and `loop.Result.TimedOut`/`DispTimeout` surface it; the orchestrator currently treats a timeout as a no-progress loop feeding stall). Then **Phase 6 (TUI)** is the main thrust: it renders the orchestrator's state/event stream (the orchestrator already persists `state.json` at every transition and surfaces a `Summary`; the `[tui]` config section (finding 14) is a prerequisite). **6.12** (derived-checklist generator) has no Bubble Tea dependency and could be pulled forward now that 4.2 produces task files.
>
...
- [x] **4.5 Per-task build flow test→build→verify.** Wire 4.4 → build loop(s) →
  test gate, per task. *Acceptance:* a task drives red→green→verified. (`07`,`06`)
  (DONE — NEW src/lib/loop/taskflow.go: Driver.RunTask drives one task test→build→verify, returning a Disposition (Done/Blocked/NoRedTest/Stalled/MaxIterations/UsageLimit/Timeout/Error/NoWork) + Verified/Iterations/StallCount for the Phase-5 orchestrator. Reads Result.TestVerdict to skip build on Satisfied / build on RedReady / re-test on NoRedTest / drain on Blocked. max-iter + stall threaded via guardrail primitives; never touches state.json. 9 tests. Version 0.0.27, tag 0.0.27.)
...
- [x] **5.1 Phase machine** `plan → build` with **drain then batch re-plan**: build
  ...
  *Audit note — iteration-budget apportionment (spec 06 OPEN):* `[guardrails]
  ...
  (DONE — `src/lib/orchestrate`. Plan-until-complete → build drain → at most one focused re-plan per drain boundary → resume, until DONE/HALTED. Iteration-budget apportionment resolved as a PER-PHASE cap (see PROGRESS 2026-05-26). Tests assert a planted gap drains → exactly one re-plan → resumes, not per-gap bouncing.)
- [x] **5.2 Full autonomy after launch** — no per-cycle approval; pause only on
  guardrail halt or usage wait. *Acceptance:* pipeline runs plan→build→done with
  no human gate. (`06`,`05`)
  (DONE — the orchestrator run loop has no per-cycle approval; it pauses only on a guardrail HALT or a usage WAIT. Wired into bare `flanders`.)
- [x] **5.3 Termination + summary.** Success when test=0 AND all tasks `done` AND
  guardrails clear → report tasks/cost/iterations/duration. *Acceptance:* summary
  emitted on completion. (`06`)
  (DONE — done iff run-level test gate green AND all tasks done AND no stall; `orchestrate.Summary` (tasks/cost/iterations/duration + halt reason) printed by `main.printSummary`.)
