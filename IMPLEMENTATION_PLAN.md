...
>
> **Next up, in priority order:** The only remaining Phase-3 open item is **3.10** (per-iteration timeout), and it is mostly a Phase-5 concern — the kill mechanism already exists in `src/lib/supervise` and `loop.Result.TimedOut` surfaces it; only the orchestrator's recording/halt remains. With 3.12 landed the entire usage-limit survival path is complete end-to-end (detect → wait/persist `WAITING` → auto-resume → `max_cycles` cap), so Phase 3 is now effectively complete except for the orchestrator-coupled 3.10. The natural next major work is therefore **Phase 4** (phases & agent classes: 4.1 subagent-class resolution, 4.2 plan loop, 4.3 plan-completeness check) and/or **Phase 5** (orchestration) — the consumer that wires 3.1–3.12 together and is also the natural home for 3.10's recording/halt.
>
