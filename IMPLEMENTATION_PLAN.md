...
>
> **Next up, in priority order:** With 4.5 done (`Driver.RunTask` drives one task test→build→verify and returns a `Disposition`), the only open Phase-4 item is **4.6** (split pass: a fresh agent turns a `blocked: context-overreach` task + handoff into 2–4 smaller tasks — `PlanIterate`/`composePlanPrompt` are a close template; reuses `plan` settings). The remaining Phase-3 item **3.10** (per-iteration-timeout recording) stays orchestrator-coupled — `supervise` already kills on timeout and `loop.Result.TimedOut`/`TaskFlowResult` (DispTimeout) surface it; the orchestrator records it. **Phase 5** (orchestration) is now the main thrust: it composes `RunTask` across tasks (drain → batch re-plan), `PlanIterate`/`PlanComplete` for the plan phase, the run-state machine (state.iter/stall/usage), and the usage `Waiter`/guardrail primitives. `RunTask` is the per-task building block it drives.
>
