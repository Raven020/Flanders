...
>
> **Next up, in priority order:** (1) **Phase 3.7 done-detection** — done iff `Verify.Passed()` AND every task `done` AND no stall; the per-loop pieces exist (verify + store.AllDone + the reconciled statuses), this composes them — but note "no stall" needs the stall counter (3.9), which needs the now-available `WorkHappened` signal, so 3.7 and 3.9 land together or 3.7 composes what exists and the orchestrator (Phase 5) owns the stall counter. (2) **Phase 3.2 prompt composition** — enrich the minimal composer with dependency outcomes + named spec excerpts. (3) **Phase 3.6 git checkpointing** — commit on progress with the templated message; this is the WRITE side that complements the new read-only `src/lib/git` (add Init/Add/Commit there).
>
...
- External dependencies: `github.com/BurntSushi/toml v1.4.0` (config parsing, TOML);
  ...
  and `src/lib/reconcile` (pure, imports task) add no external dependency.
