...
>
> **Next up, in priority order:** 3.7/3.8/3.9 are DONE (guardrail predicates, `src/lib/guardrail`). Remaining Phase-3 open items: **3.3** (loop rules file `.flanders/rules.md` — small, unblocks better agent behavior, and `loop.readRules` already reads it); **3.10** (per-iteration timeout — the kill mechanism already exists in `src/lib/supervise` + `loop.Result.TimedOut`; only orchestrator recording remains); **3.11** (context-pressure three-tier guardrail — needs `stream.Tracker` wiring + the 2.6a outbound envelope; the biggest remaining Phase-3 item); **3.12** (usage-limit wait/auto-resume — sleep/resume + state persistence). Recommended next: **(1) 3.3** (loop rules file — trivial scope, immediately improves agent prompting) or **(2) 3.11** (context-pressure — high leverage, unlocks unattended long runs).
>
